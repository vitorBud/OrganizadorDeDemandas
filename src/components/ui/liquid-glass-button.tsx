  ...
  'inline-flex cursor-pointer items-center justify-center gap-2 rounded-md text-sm font-medium whitespace-nowrap transition-colors focus-visible:ring-1 focus-visible:ring-ring focus-visible:outline-none disabled:pointer-events-none disabled:opacity-50 [&_svg]:pointer-events-none [&_svg]:size-4 [&_svg]:shrink-0',
  {
    variants: {
      variant: {
        ...
        cool: 'border-b-2 border-zinc-950/40 bg-gradient-to-t from-primary to-primary/85 text-primary-foreground shadow-md shadow-primary/20 ring-1 ring-white/25 ring-inset transition-[filter] duration-200 hover:brightness-110 active:brightness-90 dark:border-primary/50 dark:shadow-primary/30 dark:ring-white/5',
        outline:
...
const LiquidButton = React.forwardRef<HTMLButtonElement, LiquidButtonProps>(
  (
    ...
    const filterId = React.useMemo(
      () => `container-glass-${rawId.replace(/:/g, '')}`,
      [rawId]
    )
    ...
    return (
      ...
      >
        ...
        />
        <div
          className="absolute top-0 left-0 -z-10 isolate h-full w-full overflow-hidden rounded-md"
...
function metalButtonVariants(
  ...
  return {
    ...
    button: cn(
      'relative z-10 m-[1px] inline-flex h-11 transform-gpu cursor-pointer items-center justify-center overflow-hidden rounded-md px-6 py-2 text-sm leading-none font-semibold outline-none will-change-transform',
      colors.button,
      colors.textColor,
      colors.textShadow
    ),
